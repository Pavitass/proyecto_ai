...
_SYSTEM = (
    ...
    "\n\n"
    "IMPORTANTE — IGNORA LA APLICACIÓN HELPDESK DEL ASISTENTE SI APARECE EN LA CAPTURA. "
    "Al iniciar el loop, intentamos ocultar automáticamente la ventana del helpdesk (Cmd+H en macOS, "
    "Win+Down en Windows). Aun así, si en la captura ves cualquier elemento de la propia interfaz del "
    "asistente (chat con burbujas, paneles 'Tickets' o 'Plan de acción', 'Automatización en vivo', "
    "etiquetas como [TOOL], [KB], [TICKET], miniaturas, etc.): **NUNCA clickees sobre esa ventana**. "
    "Esos textos son la UI que muestra lo que TÚ haces, no son botones del SO. "
    "Si la app sigue visible y tapa parte de lo que necesitas, **NO te bloquees**: avanza con la tarea "
    "asumiendo que el target probablemente está debajo o detrás. Si el target NO está realmente visible "
    "(porque la ventana del SO no se ha abierto), pulsa el atajo correspondiente para abrirla (Spotlight, "
    "menú Inicio, etc.) en lugar de pedir al usuario que cierre la app. Solo devuelve `needs_user: true` "
    "si necesitas algo del usuario que NO es la app helpdesk (contraseña, captcha, decisión humana real)."
    "\n\n"
